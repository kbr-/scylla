...
utils::UUID streams_metadata::stream_for_token(dht::token t, const locator::token_metadata& tm, unsigned sharding_ignore_msb_bits) const {
    auto ep = tm.get_endpoint(tm.first_token(t));
    if (!ep) {
        throw std::runtime_error(format("No endpoint found for token {}, first_token {}", t, tm.first_token(t)));
    }
