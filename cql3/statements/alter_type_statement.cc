...
user_type alter_type_statement::add_or_alter::do_add(database& db, user_type to_update) const
{
    ...
    if (add_type->references_user_type(to_update->_keyspace, to_update->_name)) {
        throw exceptions::invalid_request_exception(format("Cannot add new field {} of type {} to type {} as this would create a circular reference", _field_name->name(), *_field_type, _name.to_string()));
    }
...
{
    std::optional<uint32_t> idx = to_update->idx_of_field(_field_name->name());
    if (!idx) {
    ...
    if (!new_type->is_compatible_with(*previous)) {
        throw exceptions::invalid_request_exception(format("Type {} in incompatible with previous type {} of field {} in user type {}", *_field_type, previous->as_cql3_type(), _field_name->name(), _name.to_string()));
    }
...
user_type alter_type_statement::renames::make_updated_type(database& db, user_type to_update) const
{
    ...
    for (auto&& rename : _renames) {
        ...
        std::optional<uint32_t> idx = to_update->idx_of_field(from->name());
        if (!idx) {
