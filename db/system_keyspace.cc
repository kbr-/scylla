...
load_peer_tokens_and_streams() {
    ...
    return execute_cql(req).then([] (::shared_ptr<cql3::untyped_result_set> cql_result) {
        ...
        for (auto& row : *cql_result) {
            ...
            if (row.has("tokens")) {
                auto tokens = decode_tokens(value_cast<set_type_impl::native_type>(
                        peers()->get_column_definition("tokens")->type->deserialize(row.get_blob("tokens")));
                ...
                if (row.has("streams")) {
                    streams = decode_streams(value_cast<list_type_impl::native_type>(
                            peers()->get_column_definition("streams")->type->deserialize(row.get_blob("streams")))));
                }

                ret.emplace(peer, {tokens, streams});
            }
        }
        return ret;
    });
