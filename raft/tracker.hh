...
public:
    ...
    // server.
    // More specifically, let `m` be the latest message sent by this follower among all messages
    // that we have received with term equal to ours (we may have received other messages from this
    // follower with equal term after `m`, but they were sent before `m`).
    // Let `log(m)` be the log of this follower at the moment it generated the message `m`.
    // Then our entry at `match_idx` is the same as the entry in `log(m)` at `match_idx`.
    // In particular, the terms at index `match_idx` are the same in our log and in `log(m)`.
    // By Log Matching Property we can deduce that all entries before `match_idx` are also the same.
