...
SEASTAR_TEST_CASE(test_json_udt) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        auto ut = user_type_impl::get_instance("ks", "utype", std::vector{bytes("first"), bytes("second"), bytes("third")}, {int32_type, utf8_type, float_type}, false); // TODO?
...
SEASTAR_TEST_CASE(test_unpack_decimal){
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        auto ut = user_type_impl::get_instance("ks", to_bytes("ds"),
                ...
                {decimal_type, varint_type, int32_type}, false); // TODO?
