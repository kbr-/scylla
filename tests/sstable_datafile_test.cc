...
SEASTAR_TEST_CASE(datafile_generation_11) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        collection_mutation_helper set_mut;
        set_mut.tomb = tomb;
        ...
        m.set_clustered_cell(c_key, set_col, serialize_collection_mutation(set_type, set_mut));

        auto static_set_type = static_pointer_cast<const set_type_impl>(static_set_col.type);
        m.set_static_cell(static_set_col, serialize_collection_mutation(static_set_type, set_mut));

        auto key2 = partition_key::from_exploded(*s, {to_bytes("key2")});
        mutation m2(s, key2);
        collection_mutation_helper set_mut_single;
        set_mut_single.cells.emplace_back(to_bytes("4"), make_atomic_cell(bytes_type, {}));

        m2.set_clustered_cell(c_key, set_col, serialize_collection_mutation(set_type, set_mut_single));
        ...
        auto verifier = [s, set_col, c_key] (auto& mutation) {
            ...
            return deserialize_collection_mutation(set_col.type, bv).materialize(set_col.type);
        };
